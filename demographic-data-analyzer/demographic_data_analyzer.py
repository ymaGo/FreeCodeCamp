...
def calculate_demographic_data(print_data=True):
    ...
    ratio = df[df.salary=='>50K'].groupby('native-country').count().iloc[:,0]/df.groupby('native-country').count().iloc[:,0]
    highest_earning_country = ratio.idxmax()
    highest_earning_country_percentage = ratio.sort_values(ascending=False)[0]
    highest_earning_country_percentage = round(highest_earning_country_percentage*100,1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df[(df.salary=='>50K') & (df['native-country']=='India')].groupby('occupation').count().iloc[:,0].idxmax()
